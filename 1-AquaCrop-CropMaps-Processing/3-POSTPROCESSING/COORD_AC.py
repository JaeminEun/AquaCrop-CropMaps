...
class AC_GEOreference():
    ...
    def __init__(self,):
        ...
        rows_hwsd = np.arange(0, 701)
        cols_hwsd = np.arange(0, 921)
        #self.step = (1/120)
        self.step = 0.05
        #hwsd_latini, hwsd_lonini = (-90 + 1/240),(-180 + 1/240)
        hwsd_latini, hwsd_lonini = (35.025-0.025), (-10.975-0.025)
        ...
        self.lat_ini = self.step + hwsd_latini
        self.lon_ini = self.step + hwsd_lonini
...
class AC_GEOREF2():
    "Define the latitude-longitude grid for ISIMIP and convert them to a row-column system."
    def __init__(self):
        self.lat_start = 35.025
        self.lon_start = -10.975
        self.lat_end = 70.025
        self.lon_end = 35.025
        self.step = 0.05

    def row_to_lat(self, row):
        lat = self.lat_start + (row * self.step)
        return lat

    def lat_to_row(self, lat):
        row = round((self.lat_start + lat) / self.step)
        return row

    def col_to_lon(self, col):
        lon = self.lon_start + (col*self.step)
        return lon

    def lon_to_col(self, lon):
        col = round((lon-self.lon_start)/self.step)
        return col
