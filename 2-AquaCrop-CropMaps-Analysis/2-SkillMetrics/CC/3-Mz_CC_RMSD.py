        ...
        for CGLS_row in range(i_start, i_end + 1):
            for CGLS_col in range(j_start, j_end + 1):
                ...
                if len(x_Mz_CC) > 1 and len(y_Mz_CC) > 1:
                    RMSD_value = compute_rmsd(x_Mz_CC, y_Mz_CC)
                    partial_result.append((CGLS_row, CGLS_col, RMSD_value))
...
def main():
    ...
    # Aggregate results
    RMSD_values_Mz_CC = np.full((len(lat_indices_global), len(lon_indices_global)), np.nan)
    for partial_result in results:
        if partial_result is not None:
            for (CGLS_row, CGLS_col, RMSD_value) in partial_result:
                RMSD_values_Mz_CC[CGLS_row - lat_indices_global[0], CGLS_col - lon_indices_global[0]] = RMSD_value

    RMSD_Mz_CC_da = xr.DataArray(RMSD_values_Mz_CC,
    ...
    RMSD_Mz_CC_ds = xr.Dataset({'RMSD': RMSD_Mz_CC_da})
    RMSD_Mz_CC_ds.to_netcdf('MapRMSDMz_CC.nc')
