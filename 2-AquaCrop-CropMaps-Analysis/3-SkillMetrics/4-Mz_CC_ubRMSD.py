        ...
        for CGLS_row in range(i_start, i_end + 1):
            for CGLS_col in range(j_start, j_end + 1):
                ...
                if len(x_Mz_CC) > 1 and len(y_Mz_CC) > 1:
                    ubRMSD_value = value_ubrmsd(x_Mz_CC, y_Mz_CC)
                    partial_result.append((CGLS_row, CGLS_col, ubRMSD_value))
...
def main():
    ...
    # Aggregate results
    ubRMSD_values_Mz_CC = np.full((len(lat_indices_global), len(lon_indices_global)), np.nan)
    for partial_result in results:
        if partial_result is not None:
            for (CGLS_row, CGLS_col, ubRMSD_value) in partial_result:
                ubRMSD_values_Mz_CC[CGLS_row - lat_indices_global[0], CGLS_col - lon_indices_global[0]] = ubRMSD_value

    ubRMSD_Mz_CC_da = xr.DataArray(ubRMSD_values_Mz_CC,
    ...
    ubRMSD_Mz_CC_ds = xr.Dataset({'ubRMSD': ubRMSD_Mz_CC_da})
    ubRMSD_Mz_CC_ds.to_netcdf('Map_ubRMSDMz_CC.nc')
